...
function createTributaries(horizontal, riverCount, riverWidth, waterHeight, heightRange, maxAngle, tributaryRiverTileClass, shallowTileClass, constraint)
{
	...
	for (let i = 0; i < riverCount; ++i)
	{
		...
		createArea(
			new ClumpPlacer(Math.floor(Math.PI * Math.pow(riverWidth / 2, 2)), 0.95, 0.6, 10, endX, endZ),
...
function passageMaker(x1, z1, x2, z2, width, maxHeight, shallowHeight, smooth, tileclass, terrain, riverHeight)
{
	...
	for (let ix = 0; ix < mapSize; ++ix)
		for (let iz = 0; iz < mapSize; ++iz)
		{
			...
			let my = iz - b * c / Math.pow(distance, 2);
			let inline = 0;
...
function createMountain(maxHeight, minRadius, maxRadius, numCircles, constraint, x, z, terrain, tileclass, fcc, q)
{
	...
	for (let i = 0; i < numCircles; ++i)
	{
		...
		for (let ix = sx; ix <= lx; ++ix)
		{
			...
			{
				if (Math.pow(ix - cx, 2) + Math.pow(iz - cz, 2) > radius2 || !g_Map.inMapBounds(ix, iz))
					continue;
