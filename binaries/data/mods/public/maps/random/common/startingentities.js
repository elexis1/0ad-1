...
{
	return diskArea(getDefaultPlayerTerritoryRadius());
}
...
function placeDefaultCityPatch(args)
{
	...
	createArea(
		new ClumpPlacer(
			Math.floor(diskArea(get("radiusFactor", 1/3) * get("radius", getDefaultPlayerTerritoryRadius()))),
			get("coherence", 0.6),
			get("smoothness", 0.3),
			get("failFraction", 10),
			Math.round(fx),
...
{
	let [get, fx, fz, baseResourceConstraint] = getDefaultBaseArgs(args);
	let num = Math.floor(diskArea(get("radiusFactor", 1 / 7.5) * get("radius", getDefaultPlayerTerritoryRadius())));
...
function placeDefaultDecoratives(args)
{
	...
	for (let i = 0; i < diskArea(get("radiusFactor", 1 / 15) * getDefaultPlayerTerritoryRadius()); ++i)
		for (let x = 0; x < get("maxTries", 30); ++x)
		{
...
function placePlayersRadial(radius = 0.35, startingAngle = undefined, centerX = 0.5, centerZ = 0.5)
{
...
{
	let minDist = Math.square(scaleByMapSize(60, 240));
			...
			for (let ix = 0; ix < mapSize; ++ix)
				for (let iz = 0; iz < mapSize; ++iz)
				{
					...
					for (let otherPlayer = 0; otherPlayer < i; ++otherPlayer)
						if (getDistanceSquared(ix, iz, playerX[otherPlayer], playerZ[otherPlayer]) < minDist * distFactor)
							placable = false;
