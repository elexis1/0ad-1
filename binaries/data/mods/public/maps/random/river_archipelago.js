...
var radius = scaleByMapSize(12, 20);
...
for (let i = 0; i < numPlayers; ++i)
{
	if (leftVSRight)
	{
		...
		playerX[i] = playerPos[left ? 0 : 1];
		playerZ[i] = 2 * (left ? i + 1 : numPlayers - i - 0.5) / (numPlayers + 1 + numPlayers % 2);
	}
	else
	{
		playerX[i] = playerPos[i % 2];
