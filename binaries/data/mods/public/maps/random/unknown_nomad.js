...
if (md == 1) //archipelago and island
{
	...
	let mdd1 = randIntInclusive(1, 3);
	if (mdd1 == 1) //archipelago
	{
		// create islands
		log("Creating islands...");
		placer = new ClumpPlacer(randIntExclusive(0.8 * hillSize, 1.2 * hillSize), 0.8, 0.1, 10);
	...
	else if (mdd1 == 2) //islands
	{
		// create islands
		log("Creating islands...");
		placer = new ClumpPlacer(randIntExclusive(0.6 * hillSize, 1.4 * hillSize), 0.8, 0.1, randFloat(0, 0.2));
		terrainPainter = new LayeredPainter(
			[tGrass, tGrass],		// terrains
			[2]								// widths
		);
...
else if (md == 2) //continent
{
	...
	if (randIntInclusive(1, 4) == 1) // peninsula
	{
	...
	else if (mdd1 == 2)
	{
		...
		placer = new ClumpPlacer(
			randIntInclusive(scaleByMapSize(13,24), scaleByMapSize(24,45)) *
			randIntInclusive(scaleByMapSize(13,24), scaleByMapSize(24,45)),
			0.8,
			0.1,
			10);
...
else if (md == 3) //central sea
{
	...
	{
		let x1z1 = [1, fractionToTiles(0.5)];
		let x2z2 = [fractionToTiles(0.99), fractionToTiles(0.5)];
...
else if (md == 5) //rivers and lake
{
	...
	if (randIntInclusive(1, 3) == 1 && mdd1 < 3) // island
	{
...
for (var i = 0; i < numPlayers; i++)
{
	...
	var chosen = pickRandom(placableArea);
	playerX[i] = chosen[0];
	playerZ[i] = chosen[1];
}
...
var multiplier = Math.sqrt(randFloat(0.5, 1.2) * randFloat(0.5, 1.2));
