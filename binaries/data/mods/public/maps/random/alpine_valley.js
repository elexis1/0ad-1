 ...
 *
 * To compute them, a graph is created where each vertex is a possible starting or ending location of a mountainrange and each edge a possible mountainrange.
 * The graph starts nearly complete (i.e almost every vertex is connected to most other vertices).
 * After a random edge was chosen and placed as a mountainrange,
 * edges that intersect, that leave a too small gap to other mountainranges or that are connected to too many other mountainranges are removed.
...
function MountainRangeBuilder(args)
{
	...
	/**
	 * Array of Vector2D locations where a mountainrange can start or end (vertices).
	 */
	this.points = args.points;

	/**
	 * Number of mountainranges starting or ending at the given point (degree of each vertex).
	 */
	this.vertexDegree = this.points.map(p => 0);
	...
	/**
	 * A two-dimensional array of booleans that are true if the corresponding two points may be connected by a new edge (mountainrange).
...
MountainRangeBuilder.prototype.InitPossibleEdges = function()
{
	for (let i = 0; i < this.points.length; ++i)
		for (let j = numPlayers; j < this.points.length; ++j)
			if (j > i)
...
MountainRangeBuilder.prototype.InitConnectable = function()
{
	for (let i = 0; i < this.points.length; ++i)
	{
		this.pointsConnectable[i] = [];
		for (let j = 0; j < this.points.length; ++j)
			this.pointsConnectable[i][j] = i >= numPlayers || j >= numPlayers || i == j || i != j - 1 && i != j + 1;
	}
};

MountainRangeBuilder.prototype.SetConnectable = function(isConnectable)
{
	this.pointsConnectable[this.currentEdge[0]][this.currentEdge[1]] = isConnectable;
	this.pointsConnectable[this.currentEdge[1]][this.currentEdge[0]] = isConnectable;
};

MountainRangeBuilder.prototype.UpdateCurrentEdge = function()
{
	this.currentEdge = this.possibleEdges[this.index];
	this.currentEdgeStart = this.points[this.currentEdge[0]];
	this.currentEdgeEnd = this.points[this.currentEdge[1]];
};

/**
 * Remove all edges that are too close to the current mountainrange.
 */
MountainRangeBuilder.prototype.RemoveInvalidEdges = function()
{
	for (let i = 0; i < this.possibleEdges.length; ++i)
	{
		...
		let comparedEdgeStart = this.points[comparedEdge[0]];
		let comparedEdgeEnd = this.points[comparedEdge[1]];
...
MountainRangeBuilder.prototype.HasCycles = function()
{
	...
	while (pointQueue.length)
	{
		...
		for (let i = 0; i < this.points.length; ++i)
		{
			if (this.pointsConnectable[selectedPoint][i] || i == backtree[tree.lastIndexOf(selectedPoint)])
				continue;
...
var mid = fractionToTiles(0.5);
var center = new Vector2D(mid, mid);
new MountainRangeBuilder({
	...
	"points": [
		...
		...distributePointsOnCircle(numPlayers, startAngle + Math.PI / numPlayers, fractionToTiles(0.49), center.x, center.y)[0],
		...distributePointsOnCircle(numPlayers, startAngle + Math.PI / numPlayers * 1.4, fractionToTiles(0.34), center.x, center.y)[0],
		...distributePointsOnCircle(numPlayers, startAngle + Math.PI / numPlayers * 0.6, fractionToTiles(0.34), center.x, center.y)[0],
		...distributePointsOnCircle(numPlayers, startAngle + Math.PI / numPlayers, fractionToTiles(0.18), center.x, center.y)[0],
		center
	]
