...
function scrollBackgrounds()
{
	for (let i = 0; i < g_BackgroundLayerset.length; ++i)
	{
		...
		let time = (Date.now() - g_T0) / 1000;
		let offset = g_BackgroundLayerset[i].offset(time, w);
...
{
	let topSprite = Engine.GetGUIObjectByName("MainMenuPanelRightBorderTop");
	topSprite.size = "100%-2 0 100% " + (topPosition + MARGIN);

	let bottomSprite = Engine.GetGUIObjectByName("MainMenuPanelRightBorderBottom");
