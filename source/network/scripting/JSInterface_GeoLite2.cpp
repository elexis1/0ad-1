...
std::vector<GeoLite2Data> JSI_GeoLite2::GetGeoLite2(ScriptInterface::CxPrivate* UNUSED(pCxPrivate), const std::string& ipAddress)
{
	if (g_GeoLite2)
	{
		u32 ipAddressNum;
		if (IPTools::ParseIPv4Address(ipAddress, ipAddressNum))
			return g_GeoLite2->GetIPv4Data(ipAddressNum);

		// TODO: Support IPv6
	}
	return std::vector<GeoLite2Data>();
}
...
void JSI_GeoLite2::RegisterScriptFunctions(const ScriptInterface& scriptInterface)
{
	...
	scriptInterface.RegisterFunction<std::vector<GeoLite2Data>, std::string, &GetGeoLite2>("GetGeoLite2");
}
